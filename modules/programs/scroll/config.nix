	...
	home-manager = {
		...
		users.${vars.user.name} = {...}: {
			xdg.configFile."scroll/config".text = ''
				...
				# exec /usr/lib/xdg-desktop-portal-termfilechooser
				...
				output DVI-D-1 resolution 1600x900 position 0,0
				#
				...
				# workspace_next_on_output_create_empty false
				xwayland_output_scale false
				# focus_wrapping yes
				...
				#
				    ...
				    # Kill focused window
				    bindsym $mod+q kill
				    # bindsym $mod+Shift+q kill unfocused
				    # bindsym $mod+Ctrl+q kill all
				    ...
				    bindsym ctrl+$mod+alt+q exec scrollnag -t warning -m 'You pressed the exit shortcut. Do you really want to exit scroll? This will end your Wayland session.' -B 'Yes, exit scroll' 'scrollmsg exit' -w 500 -e center -y overlay

				    # Layout mode
				    bindsym $mod+alt+$left set_mode h before
				    bindsym $mod+alt+$right set_mode h after
				    bindsym $mod+alt+$up set_mode v before
				    bindsym $mod+alt+$down set_mode v after
				#
				# Moving around:
				#
